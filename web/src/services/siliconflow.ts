...
const SYSTEM_PROMPT = '你是一个信息抽取器。只输出 JSON，不要输出解释、不要 Markdown 代码块。';

function buildUserPrompt(text: string): string {
  const today = new Date().toISOString().slice(0, 10);
  return `从文本中提取所有物品信息并输出 JSON 数组（字段缺失用 null）。
今天日期：${today}
规则：
...
- 运动户外：运动衣、速干衣、运动裤、瑜伽裤、运动鞋、运动包、瑜伽垫、哑铃、拉力器、仰卧板、跑步机、动感单车、篮球、足球、羽毛球、乒乓球、跳绳、帐篷、睡袋、登山包、渔具、手电筒、头灯、钥匙扣、多功能钥匙扣、打火机、防风打火机、求生刀、瑞士军刀、折叠刀、多功能工具、指南针、急救包、防水袋、户外水壶、登山杖
...
function parseAIResponse(content: string): AIExtractedItem[] {
...
export async function analyzeText(text: string, apiKey: string): Promise<AIExtractedItem[]> {
